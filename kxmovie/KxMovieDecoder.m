...
- (KxAudioFrame *) handleAudioFrame//ffmepg获取音频frame
{
...
- (NSArray *) decodeFrames: (CGFloat) minDuration//ffmpeg获取frame
{
    if (_videoStream == -1 &&
        _audioStream == -1)
        return nil;
    ...
    while (!finished) {
        ...
        if (av_read_frame(_formatCtx, &packet) < 0) {//ffmpeg, 读取方法
            _isEOF = YES;
            break;
        }
        ...
        if (packet.stream_index ==_videoStream) {//视频流
            ...
            while (pktSize > 0) {
                ...
                int gotframe = 0;
                int len = avcodec_decode_video2(_videoCodecCtx,//全局ffmpeg参数配置
                                                _videoFrame,
                                                &gotframe,
                                                &packet);//视频解码
