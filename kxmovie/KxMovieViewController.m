...
{    
    ...
    [audioManager activateAudioSession];//音频、只有注册block
...
//    @"http://192.168.78.115:8080/live/vr.m3u8"
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (strongSelf) {
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    ...
                    [strongSelf setMovieDecoder:decoder withError:error];   //初始化视频——setupPresentView
                });
            }
        });
    }
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    if (_decoder) {
        ...
        [self restorePlay];//play_打开循环addframes_tick(addFrames)
...
- (BOOL) addFrames: (NSArray *)frames
{
    if (_decoder.validVideo) {
        ...
        @synchronized(_videoFrames) {
            ...
            for (KxMovieFrame *frame in frames)
                if (frame.type == KxMovieFrameTypeVideo) {//视频解码
                    [_videoFrames addObject:frame];//_videoFrames:视频frame集合
                    _bufferedDuration += frame.duration;
                }
        }
    }
    ...
    if (_decoder.validAudio) {
        ...
        @synchronized(_audioFrames) {
            ...
            for (KxMovieFrame *frame in frames)
                if (frame.type == KxMovieFrameTypeAudio) {//音频解码
